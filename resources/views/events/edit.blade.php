                    ...
                    <div class="border rounded-md border-[#EDEDED] w-full h-[178px] ml-6 mt-3 p-6">
                        <div class="border rounded-sm border-dashed border-[#BABABA] flex flex-col justify-center items-center h-full gap-3">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                ...
                                <path d="M13 2H9C4 2 2 4 2 9V15C2 20 4 22 9 22H15C20 22 22 20 22 15V10" stroke="#9A9A9A"
                                ...
                                <path d="M2.67 18.9501L7.6 15.6401C8.39 15.1101 9.53 15.1701 10.24 15.7801L10.57 16.0701C11.35 16.7401 12.61 16.7401 13.39 16.0701L17.55 12.5001C18.33 11.8301 19.59 11.8301 20.37 12.5001L22 13.9001"
                                      stroke="#9A9A9A" stroke-width="1.5" stroke-linecap="round"
                                      stroke-linejoin="round"/>
                            </svg>

                            <input type="file" name="image" id="image" placeholder="Event Image"
                                   class="font-work text-sm font-medium text-[#9A9A9A] border rounded-sm"/>
                            @error('image')
                ...
                <div class="mt-5">
                    ...
                    <div class="border rounded-md border-[#EDEDED] w-full h-auto p-6 flex flex-col gap-5 ml-6 mt-3">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{$event->name}}" placeholder="Max. 10 words"
                                   class="border rounded-md border-[#EDEDED] focus:border-[#3C4D27] placeholder-[#9A9A9A] font-work outline-0 px-6 py-3 w-full text-sm"/>
                            @error('name')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="flex gap-14">
                            <div>
                                ...
                                <input type="date" name="date" id="date" value="{{date('Y-m-d', strtotime($event->date))}}"
                                       class="border border-[#EDEDED] focus:border-[#3C4D27] rounded-md outline-0 font-work px-6 py-3 h-10 text-sm"/>
                                @error('date')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <input type="number" name="quota" id="quota" value="{{$event->quota}}"
                                       class="border border-[#EDEDED] focus:border-[#3C4D27] rounded-md font-work placeholder-[#9A9A9A] outline-0 px-6 py-3 h-10 text-sm"
                                       placeholder="Min. 10 persons"/>
                                @error('quota')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                    class="bg-hijau-100 rounded-md font-work text-white text-base px-4 py-2">
                                Edit
                            </button>
                        </div>
                    </div>
